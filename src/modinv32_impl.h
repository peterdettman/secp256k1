...
/* Optionally negate a signed30 number in range (-2*modulus,modulus), and add multiple of modulus to
 * bring it to [0,modulus). The input must have limbs in range (-2^30,2^30). The output will have
 * limbs in range [0,2^30). */
static void secp256k1_modinv32_normalize_30(secp256k1_modinv32_signed30 *r, int32_t cond_negate, const secp256k1_modinv32_modinfo *modinfo) {
     ...
     * limbs are in range (-2^30,2^30), this cannot overflow an int32_t. */
    cond_add = r8 >> 31;
    ...
    VERIFY_CHECK(r->v[0] >> 30 == 0);
    VERIFY_CHECK(r->v[1] >> 30 == 0);
    VERIFY_CHECK(r->v[2] >> 30 == 0);
    VERIFY_CHECK(r->v[3] >> 30 == 0);
    VERIFY_CHECK(r->v[4] >> 30 == 0);
    VERIFY_CHECK(r->v[5] >> 30 == 0);
    VERIFY_CHECK(r->v[6] >> 30 == 0);
    VERIFY_CHECK(r->v[7] >> 30 == 0);
    VERIFY_CHECK(r->v[8] >> 16 == 0);
#endif
}
 ...
 */
static uint32_t secp256k1_modinv32_divsteps_30(uint32_t eta, uint32_t f0, uint32_t g0, secp256k1_modinv32_trans2x2 *t) {
 ...
 */
static uint32_t secp256k1_modinv32_divsteps_30_var(uint32_t eta, uint32_t f0, uint32_t g0, secp256k1_modinv32_trans2x2 *t) {
